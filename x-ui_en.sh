...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/alixtron0/xtron-vpn/main/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/alixtron0/xtron-vpn/edit/main/x-ui_en.sh
    if [[ $? != 0 ]]; then
        echo ""
